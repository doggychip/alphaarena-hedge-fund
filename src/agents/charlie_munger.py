...
def charlie_munger_agent(state: AgentState, agent_id: str = "charlie_munger_agent"):
    ...
    for ticker in tickers:
        if is_crypto(ticker):
            ...
            if crypto_data:
                progress.update_status(agent_id, ticker, "Generating Charlie Munger crypto analysis")
                output = generate_crypto_persona_signal(
                    ticker=ticker, crypto_data=crypto_data,
                    persona_system_prompt="You are Charlie Munger. Apply mental models, inversion thinking, and rationality. Focus on moat quality, management, predictability, and avoiding stupidity. Be candid about whether crypto fits your framework. Keep reasoning under 200 characters.",
                    signal_model=CharlieMungerSignal, default_factory=lambda: CharlieMungerSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                munger_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
