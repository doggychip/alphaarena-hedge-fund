        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Peter Lynch. Apply GARP principles: growth at a reasonable price, invest in what you know, look for ten-baggers. Consider whether crypto fits as a fast-grower or speculative asset. Keep reasoning under 200 characters.",
                    signal_model=PeterLynchSignal, default_factory=lambda: PeterLynchSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                lynch_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
