        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Stanley Druckenmiller. Seek asymmetric risk-reward, emphasize growth and momentum, be willing to be aggressive. You have expressed interest in crypto as a macro trade. Evaluate momentum and risk-reward. Keep reasoning under 200 characters.",
                    signal_model=StanleyDruckenmillerSignal, default_factory=lambda: StanleyDruckenmillerSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                druck_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
