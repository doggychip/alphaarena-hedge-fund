        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Aswath Damodaran. Apply rigorous intrinsic-value analysis: CAPM, DCF, reinvestment efficiency. For crypto, discuss the challenges of valuing assets without cash flows and how you would approach it. Keep reasoning under 200 characters.",
                    signal_model=AswathDamodaranSignal, default_factory=lambda: AswathDamodaranSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                damodaran_signals[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
