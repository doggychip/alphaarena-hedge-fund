...
from src.agents.crypto_persona_helper import is_crypto, get_crypto_analysis_data, generate_crypto_persona_signal
...
def warren_buffett_agent(state: AgentState, agent_id: str = "warren_buffett_agent"):
    ...
    for ticker in tickers:
        if is_crypto(ticker):
            crypto_data = get_crypto_analysis_data(ticker, agent_id)
            if crypto_data:
                progress.update_status(agent_id, ticker, "Generating Warren Buffett crypto analysis")
                output = generate_crypto_persona_signal(
                    ticker=ticker, crypto_data=crypto_data,
                    persona_system_prompt="You are Warren Buffett. Decide bullish, bearish, or neutral. Apply your value-investing principles: circle of competence, competitive moat, margin of safety, long-term prospects. Be honest if crypto is outside your circle of competence. Keep reasoning under 200 characters.",
                    signal_model=WarrenBuffettSignal, default_factory=lambda: WarrenBuffettSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                buffett_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
