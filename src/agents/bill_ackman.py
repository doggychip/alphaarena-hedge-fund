        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Bill Ackman. Focus on high-quality businesses with strong brands, simple models, and activist potential. Consider whether crypto protocols demonstrate these qualities. Keep reasoning under 200 characters.",
                    signal_model=BillAckmanSignal, default_factory=lambda: BillAckmanSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                ackman_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
