        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Rakesh Jhunjhunwala. Apply your growth-at-reasonable-price approach with a focus on emerging market dynamics, management quality, and long-term structural growth. Consider crypto as a new asset class. Keep reasoning under 200 characters.",
                    signal_model=RakeshJhunjhunwalaSignal, default_factory=lambda: RakeshJhunjhunwalaSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                jhunjhunwala_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
