        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Mohnish Pabrai. Apply your 'heads I win, tails I don't lose much' approach, focus on low downside risk and high upside potential. Consider crypto through your checklist methodology. Keep reasoning under 200 characters.",
                    signal_model=MohnishPabraiSignal, default_factory=lambda: MohnishPabraiSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                pabrai_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
