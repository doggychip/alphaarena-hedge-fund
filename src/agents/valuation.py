...
def valuation_analyst_agent(state: AgentState, agent_id: str = "valuation_analyst_agent"):
    ...
    for ticker in tickers:
        if detect_asset_type(ticker) == AssetType.CRYPTO:
            result = _analyze_crypto_valuation(ticker, agent_id)
            if result:
                valuation_analysis[ticker] = result
            continue
...
def _analyze_crypto_valuation(ticker: str, agent_id: str) -> dict | None:
    """Analyze crypto valuation using NVT-like ratio, historical range, and market cap dominance."""
    progress.update_status(agent_id, ticker, "Fetching crypto data for valuation")
    ...
    # 1. NVT-like ratio (Network Value to Transaction ratio proxy)
    # Use volume/market_cap as inverse proxy — higher = more transaction activity relative to value
    vol_mcap = crypto_data.volume_to_market_cap
    if vol_mcap is not None:
        if vol_mcap > 0.15:
            signals.append("bullish")  # High activity relative to valuation
        elif vol_mcap > 0.05:
            signals.append("neutral")
        else:
            signals.append("bearish")  # Low activity, potentially overvalued
    else:
        signals.append("neutral")
    reasoning["nvt_proxy"] = {
        "signal": signals[-1],
        "details": f"Volume/MCap (NVT proxy): {vol_mcap:.4f}" if vol_mcap else "NVT proxy: N/A",
    }

    # 2. ATH-based historical range valuation
    ath = crypto_data.ath
    current_price = crypto_data.current_price
    ath_change = crypto_data.ath_change_percentage
    if ath is not None and current_price is not None and ath > 0:
        price_to_ath_ratio = current_price / ath
        if price_to_ath_ratio < 0.3:
            signals.append("bullish")  # Deeply discounted from ATH
        elif price_to_ath_ratio < 0.7:
            signals.append("neutral")
        else:
            signals.append("bearish")  # Near ATH, limited upside
    else:
        signals.append("neutral")
        price_to_ath_ratio = None
    reasoning["historical_range"] = {
        "signal": signals[-1],
        "details": f"Price/ATH: {price_to_ath_ratio:.2%}, ATH Change: {ath_change:.1f}%" if price_to_ath_ratio is not None and ath_change is not None else "Historical range: N/A",
    }

    # 3. Market cap dominance / rank as relative value indicator
    rank = crypto_data.market_cap_rank
    market_cap = crypto_data.market_cap
    if rank is not None and market_cap is not None:
        # Top coins tend to be more fairly valued; lower-rank coins may be over/undervalued
        if rank <= 5:
            signals.append("neutral")  # Large caps are typically efficiently priced
        elif rank <= 20:
            signals.append("bullish")  # Mid-cap, potential for growth
        else:
            signals.append("bearish")  # Higher risk, potentially speculative valuation
    else:
        signals.append("neutral")
    reasoning["market_position_value"] = {
        "signal": signals[-1],
        "details": f"Rank: #{rank}, MCap: ${market_cap:,.0f}" if rank and market_cap else "Market position: N/A",
    }

    # 4. Supply-based valuation (circulating vs total supply)
    ...
    if circ is not None and total is not None and total > 0:
        ...
        if supply_ratio > 0.85:
            signals.append("bullish")  # Most supply already circulating, less dilution risk
        elif supply_ratio > 0.5:
            signals.append("neutral")
        else:
            signals.append("bearish")  # Lots of supply yet to enter market
    else:
        signals.append("neutral")
        supply_ratio = None
    reasoning["supply_analysis"] = {
        "signal": signals[-1],
        "details": f"Circulating/Total: {supply_ratio:.1%}" if supply_ratio is not None else "Supply data: N/A",
    }
