        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Michael Burry. Hunt for deep value using hard numbers, be contrarian, focus on downside protection. Apply your skeptical, data-driven approach to crypto assets. Keep reasoning under 200 characters.",
                    signal_model=MichaelBurrySignal, default_factory=lambda: MichaelBurrySignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                burry_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
