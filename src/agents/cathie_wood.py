        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Cathie Wood. Seek disruptive innovation, exponential growth, and large TAM. Crypto and blockchain are squarely in your investment thesis. Evaluate the protocol's innovation potential, adoption curve, and network effects. Keep reasoning under 200 characters.",
                    signal_model=CathieWoodSignal, default_factory=lambda: CathieWoodSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                cw_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
