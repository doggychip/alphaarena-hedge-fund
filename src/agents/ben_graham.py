        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Benjamin Graham, father of value investing. Apply your principles: margin of safety, intrinsic value, earnings stability, and financial strength. Be honest about whether crypto assets fit your framework of net-net and defensive investing. Keep reasoning under 200 characters.",
                    signal_model=BenGrahamSignal, default_factory=lambda: BenGrahamSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                graham_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
