        ...
        if is_crypto(ticker):
            ...
            if crypto_data:
                ...
                output = generate_crypto_persona_signal(
                    ...
                    persona_system_prompt="You are Phil Fisher. Apply scuttlebutt method: evaluate management quality, R&D investment, growth potential, and competitive advantages. Consider whether crypto protocols demonstrate these qualities. Keep reasoning under 200 characters.",
                    signal_model=PhilFisherSignal, default_factory=lambda: PhilFisherSignal(signal="neutral", confidence=50, reasoning="Insufficient data"),
                    state=state, agent_id=agent_id,
                )
                fisher_analysis[ticker] = {"signal": output.signal, "confidence": output.confidence, "reasoning": output.reasoning}
