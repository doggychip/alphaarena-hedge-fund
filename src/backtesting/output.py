    ...
    def build_day_rows(
        ...
        *,
        ...
        for ticker in tickers:
            ...
            date_rows.append(
                format_backtest_row(
                    date=date_str, ticker=ticker, action=action, quantity=quantity,
                    price=current_prices[ticker], long_shares=pos["long"],
                    short_shares=pos["short"], position_value=net_position_value,
                )
            )
        ...
        summary = compute_portfolio_summary(
            ...
            initial_value=initial_value, performance_metrics=performance_metrics,
        )

        date_rows.append(
            format_backtest_row(
                date=date_str, ticker="", action="", quantity=0, price=0,
                long_shares=0, short_shares=0, position_value=0, is_summary=True,
                total_value=summary["total_value"], return_pct=summary["return_pct"],
                cash_balance=summary["cash_balance"],
                total_position_value=summary["total_position_value"],
                sharpe_ratio=summary["sharpe_ratio"], sortino_ratio=summary["sortino_ratio"],
                max_drawdown=summary["max_drawdown"], benchmark_return_pct=benchmark_return_pct,
            )
        )
        return date_rows
